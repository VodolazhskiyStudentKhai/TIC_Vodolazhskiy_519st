...
def plot(x, y, axis_x="", axis_y="", title=""):
    ...
    isdir = os.path.isdir(f'{path}')
    if not isdir:
        os.mkdir(f'{path}')
    fig.savefig(f'{path}' + title + '.png', dpi=600)
    plt.close(fig)
...
def fsk_modulation(freq1, freq2, seq):
    sequence_fsk = np.zeros(1000)
    for i in range(0, len(seq)):
        sequence_fsk[i] = seq[i] * sin(2 * pi * freq1 * i / 1000) + (abs(seq[i] - 1)) * sin(2 * pi * freq2 * i / 1000)
    return sequence_fsk
...
def noise_stress(sequence, sequence_modulated, modulation, frequency):
    ...
    for i in range(0, 20):
        ...
        for m in range(0, 200):
            ...
            seq_noise = sequence_modulated + i * noise
            if modulation == "ASK":
                ask_demodulated_signal, sequence_demodulated = ask_demodulation(frequency[0], seq_noise)
            elif modulation == "PSK":
                psk_demodulated_signal, sequence_demodulated = psk_demodulation(frequency[0], seq_noise)
            elif modulation == "FSK":
                fsk_demodulated_signal1, fsk_demodulated_signal2, sequence_demodulated =\
                    fsk_demodulation(frequency[0], frequency[1], seq_noise)
...
def start_modulation(ask, psk, fsk1, fsk2):
    ...
    noise = create_noise(0, 1, 1000)
    seq_ask_noise = sequence_ask + noise
    seq_psk_noise = sequence_psk + noise
    seq_fsk_noise = sequence_fsk + noise
    plot(x, seq_ask_noise, axis_x="Час, c", axis_y="Амплітуда сигналу", title="Амплітудна модуляція з шумом")
    plot(x, seq_psk_noise, axis_x="Час, c", axis_y="Амплітуда сигналу", title="Фазова модуляція з шумом")
    plot(x, seq_fsk_noise, axis_x="Час, c", axis_y="Амплітуда сигналу", title="Частотна модуляція з шумом")
    ask_demodulated_signal, sequence_demodulated = ask_demodulation(ask, seq_ask_noise)
    ...
    psk_demodulated_signal, sequence_demodulated = psk_demodulation(psk, seq_psk_noise)
    ...
    fsk_demodulated_signal1, fsk_demodulated_signal2, sequence_demodulated = fsk_demodulation(fsk1, fsk2, seq_fsk_noise)
    ...
    if not isdir:
        os.mkdir(f'{path}')
    fig.savefig(f'{path}' + 'Оцінка завадостійкості трьох видів модуляції' + '.png', dpi=600)
    plt.close(fig)


start_modulation(variant[0], variant[1], variant[2], variant[3])
